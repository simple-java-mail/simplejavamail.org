...
I am not an expert on DKIM, S/MIME, object-pooling semantics, SOCKS, Outlook's file format, RTF or email-address validation. You will probably never find me at a conference teaching the finer points of the MIME standards. I do know how to take something complex and daunting and make it feel manageable. I can divide it into parts, decide which parts a user actually needs to see, and find what is, in my opinion, the right balance between a useful API and escape hatches for the cases I cannot predict. I basically pretend to be a user looking for the easiest way to do something.

That is my power. This sounds grand when I write it down, but in practice it often looks like me reading a half-understood codebase at night, renaming everything, adding tests and trying to work out which three concepts should survive into its public API. This has been my cycle since even before I went to college.
...
flowchart
    ...
    family["Supporting-library family"]
...
At first, these were dependencies in a `pom.xml`: code somebody else would keep working while I concentrated on Simple Java Mail. That is a comforting fiction until a dependency is archived, a licence blocks the next release, a user finds a production leak across four repositories, or a mail client invents another way to encode the same body.
...
flowchart LR
    ...
    bundled["Bundled in<br/>Vesijama / JSM<br/>2009–2016"]
...
## A parser was hiding inside a contribution
...
S/MIME has a longer chain. JavaMail-Crypto started on [SourceForge](/sources/sourceforge/javamail-crypto/project.html) in 2003 and covered both S/MIME and OpenPGP. Its archives contain the same sort of real-world friction: a signature [accepted by Outlook and Thunderbird but rejected by webMethods](/sources/sourceforge/javamail-crypto/mailman/message-457552.html#message-457552), and an old bug where [`setHeader` overwrote repeated headers](/sources/sourceforge/javamail-crypto/bugs/3.html#ticket-3) such as multiple `To` fields. Markenwerk extracted and modernised the S/MIME part in [2015](https://github.com/markenwerk/java-utils-mail-smime/commit/2ae3534db52e00bd8427cdc68ed83fef4f0d8066).

By 2021 that fork was archived. Users were [stranded behind ignored fixes](https://github.com/bbottema/simple-java-mail/issues/334), and the move from `javax.mail` to Jakarta Mail could not happen while it remained there. Taking it over required permission from three parties across those generations. In June I had [two out of three](https://github.com/bbottema/simple-java-mail/issues/295#issuecomment-869926957). By Christmas I had all of them, and on 28 December the project was [migrated and relicensed](https://github.com/simple-java-mail/java-utils-mail-smime/commit/6eac0beb12506b9dbfb345431ab3f1de05ffb5e4). Simple Java Mail 7.0.0 followed days later. The stop-start part of that same issue belongs to [the story about my sabbaticals](/journal/the-library-i-keep-coming-back-to.html); here, the important part is that succession took three separate permissions rather than one convenient fork button.

My job here was to preserve the correct boundary: signing and encryption are not random `MimeMessage` tricks, they are a separate domain with their own keys, algorithms, verification results and failure modes. Then I had to make sure the people who did create that implementation were carried into its history and that the library could keep moving.
...
flowchart LR
    ...
    accDescr: Youchao Feng's io.github.fengyouchao:sockslib was adapted into Simple Java Mail in 2016 and later moved into org.simplejavamail:authenticated-socks-module. Separately, code from Jose Ricardo de Oliveira Damico's org.jdamico.socks.server became com.github.bbottema:java-socks-proxy-server in 2019, with the original author's permission recorded in 2024.
    ...
    testSource["Jose Ricardo Damico<br/>org.jdamico.socks.server<br/>2014"]
...
Somewhere in that overhaul I also dropped Jeremy's attribution. My memory had suggested a licence incompatibility, but the repository says otherwise. KBOP's final [licence file is MIT](https://github.com/gondor/kbop/blob/a176fd845f1e146610f03a1cb3cb0d661ebf4faa/LICENSE.txt), while its stale [POM still says Apache 2.0](https://github.com/gondor/kbop/blob/a176fd845f1e146610f03a1cb3cb0d661ebf4faa/pom.xml#L18-L31). Neither explains leaving the provenance out of my new repository. Project archaeology occasionally turns up maintenance work you owe the past. This is one of those cases.

That paragraph became outdated almost immediately. This agent-powered archaeology dig identified the original project and the surviving code precisely enough that on 2 September 2026 I [restored Jeremy and KBOP's attribution and added the MIT licence](https://github.com/bbottema/generic-object-pool/commit/25f044ca87e9d1bf677eb885d91bc1f548d0c768). The public trail dates my KBOP modernisation work to June 2019, but the first Generic Object Pool commit contains the already-overhauled project, not a record of when I made the fork. Apparently agent-assisted maintenance can include sending you back to finish a job you did not realise was still open.
...
That stack has kept teaching the same lesson. A source IP and a destination domain turned out to need [different cluster and pool keys](https://github.com/bbottema/clustered-object-pool/issues/1). Global defaults inherited from the first `Mailer` contaminated [later clusters](https://github.com/bbottema/simple-java-mail/issues/565). A request to ping idle connections became a more general [`claimMatching` boundary](https://github.com/bbottema/generic-object-pool/issues/11), because the generic pool should expose facts while the SMTP layer owns network policy. Another request preserved the pool's honest [synchronous allocation contract](https://github.com/bbottema/generic-object-pool/issues/12), while the SMTP layer grew a Jakarta Mail [provider integration](https://github.com/simple-java-mail/smtp-connection-pool/issues/10) that has to delegate without recursively selecting itself. Each fix made the compartments a little more honest.

## What I actually bring to the stack

There is a version of this history in which I apologise for not being an expert in all the underlying topics. I do not think that is the right conclusion. Not being the specialist in every underlying topic does not mean I do not know how to code.

I have been developing software since I was fourteen: first (D)HTML, Flash and ActionScript, then Delphi, followed by Java and even Assembly while studying Computer Science. In the twenty years since graduating I have kept writing Java, with Python, Dart, a lot of Kotlin, JavaScript, TypeScript and everything around them along the way. The point is not that I cannot write the internals. It is that no one person can be the deepest expert on all of MIME, cryptography, pooling, proxies, Outlook and RTF.

What I bring is engineering, architecture and developer UX together. Libraries have UX too; Joshua Bloch gave [a whole talk about making APIs easy to use and hard to misuse](https://www.infoq.com/presentations/effective-api-design/). He has also been candid about [the "obvious API design mistakes" even he and Doug Lea make](https://www.artima.com/articles/josh-bloch-on-design). I can recognise the useful centre of a project, make its shape smaller, connect it to a larger system and keep the control surface proportional to what users actually need. The aim is a robust library without making every user fly it from a Boeing cockpit. Otherwise Simple Java Mail, whose mission is to hide the low-level MIME standards, would defeat itself.
...
Coding agents fit me so well because they amplify this particular strength. They are not working from a blank page either. Over nineteen years, the project has built up momentum and established development routes for agents to follow, providing a strong harness for their work. I can give them the boundary, the history, the invariants and the integration point, then make them focus on a protocol or some parser until they get the details and semantics right. I still have to review that work and own the public promise. But I no longer have to become a world authority on every compartment before I can improve it.

I am still a hack; bringing the whole thing together is the part I am actually good at. Software Engineer With A Hacksaw sounds closer to the truth. Hmm, maybe I should put that on my LinkedIn.
