...
5. **Set message rules once:** defaults, forced overrides, validation, and default signing or encryption where applicable.
...
1. **Security at a glance:** link cards for TLS, OAuth2, DKIM, S/MIME, OpenPGP/MIME, and header-injection protection. Keep connection factories, individual transport variants, trust settings, and other lower-level controls in the table of contents.
2. **Choose the right layer:** a compact matrix compares transport encryption, SMTP authentication, domain authentication, content signing/encryption, and input validation. It states whether each route contributes to DMARC.
3. **Choose a transport strategy:** SMTP, SMTPS, SMTP with STARTTLS, OAuth2; explain what each does and does not protect.
4. **Verify the server:** trusted hosts, server identity, custom SSL factory, certificate behavior.
5. **Prevent header injection:** CRLF handling and where validation occurs.
6. **Sign with DKIM:** per-message and mailer defaults, selectors/keys, optional module note.
7. **Sign and encrypt with S/MIME:** signing, encryption, per-recipient certificates, reading/merging/decrypting, optional module note.
8. **Sign and encrypt with OpenPGP/MIME:** in-memory key rings, signing, multi-recipient encryption, verification, decryption, status handling, optional module note.
9. **Security checklist:** secrets, TLS verification, key handling, logging, tests.

Preserve all current transport-strategy, SSL, trusted-host, server-identity, CRLF, DKIM, and S/MIME anchors. Add stable anchors for the security-route matrix and OpenPGP sending and reading.
...
- Explain that OpenPGP/MIME and S/MIME protect the message content through different key and trust systems. They are alternatives for one message in 10.0.
- Explain OAuth2 as an authentication mechanism, not a transport-encryption substitute.
- Explain DMARC only where it helps the developer choose a route: aligned DKIM can satisfy DMARC; SPF and DMARC policy live outside the library; S/MIME, OpenPGP/MIME, TLS, and OAuth2 do not count toward DMARC.
- Avoid blanket “secure” claims without naming the layer.

### Metadata intent

Title: `Secure Java email with TLS, OAuth2, DKIM, S/MIME, and OpenPGP`
Description: `Configure Simple Java Mail transport verification, OAuth2, CRLF protection, DKIM signing, and S/MIME or OpenPGP/MIME signing and encryption.`
...
- Clearly name the required optional module before DKIM, S/MIME, OpenPGP/MIME, or batch examples.
